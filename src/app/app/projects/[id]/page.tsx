...
type Link = {
  ...
  from_id: string;
  to_id: string;
  kind: string;                   // 'precedes' | 'data' (etc.)
  source_handle?: string | null;
  target_handle?: string | null;
  curve?: string | null;          // 'step' | 'smooth'
  pattern?: string | null;        // 'solid' | 'dashed' | 'dotted'
...
function Inner() {
  ...
  const colorForKind = (k: K) => {
    ...
    if (k === 'choice') return { bg: '#F6E7B2', textColor: '#1f2937', fold: 'rgba(0,0,0,.14)' }; // gold-ish
    return { bg: '#FFF7B3', textColor: '#1f2937', fold: 'rgba(0,0,0,.14)' };
  };
      ...
      prev.map((n: any) =>
        n.id !== id
          ...
          : {
              ...
              data: {
                ...n.data,
                label: (n.type as string) === 'choice' ? 'Choice: ' + title : title
              }
            }
      )
    );
  ...
  const load = useCallback(async () => {
    ...
    setNodes(
      ds.map(d => ({
        ...
        data: {
          label: d.kind === 'choice' ? `Choice: ${d.title || 'New'}` : d.title || 'New',
          onRename,
    ...
    setEdges(
      ls.map(l => {
        const dash =
          l.pattern === 'dotted' ? '2 6' : l.pattern === 'dashed' ? '8 6' : undefined;
        return {
          id: l.id,
          source: l.from_id,
          target: l.to_id,
        ...
        if (data) {
          ...
          setNodes(prev => [
            ...prev,
            {
              ...
              data: {
                label: d.kind === 'choice' ? `Choice: ${d.title}` : d.title,
                onRename,
                onTabCreate: handleTabCreate,
                isEditing: false,
                ...colorForKind(d.kind)
  ...
  const onDrop = useCallback(
    (event: React.DragEvent) => {
      ...
      // new long-term key for tools
      const kind = event.dataTransfer.getData('application/dtl-kind') as K;
      if (!kind) return;
          ...
          onChange={(upd: any) => {
            setEdges((eds: any) =>
              eds.map((e: any) => {
                ...
                const dash =
                  upd.data?.pattern === 'dotted'
                    ? '2 6'
                    : upd.data?.pattern === 'dashed'
                    ? '8 6'
                    : undefined;
                return {
                  ...e,
