...
import type { Decision, Kind, Link, NodeData, EdgeData } from '@/types/canvas';
...
function Inner() {
  ...
  function colorForKind(k: Kind): { bg: string; textColor: string; fold: string } {
        ...
        prev.map((n) =>
          n.id !== id
            ...
            : {
                ...
                data: {
                  ...n.data,
                  label: ((n.type as Kind) === 'choice' ? 'Choice: ' : '') + title,
                },
              }
        )
      );
  ...
  const load = useCallback(async () => {
    ...
    setEdges(
      ls.map<FlowEdge<EdgeData>>((l) => {
        const markerStart = l.arrow_start ? { type: MarkerType.ArrowClosed } : undefined;
        const markerEnd = l.arrow_end !== false ? { type: MarkerType.ArrowClosed } : undefined;
        const dash =
  ...
  const onNodeDragStop = useCallback(async (_e: unknown, node: FlowNode<NodeData>) => {
    await supabase.from('decisions').update({ x: node.position.x, y: node.position.y }).eq('id', node.id);
  }, []);

  const onNodeDoubleClick = useCallback(
    (_e: unknown, n: FlowNode<NodeData>) => {
  ...
  const onEdgeDoubleClick = useCallback((_e: unknown, edge: FlowEdge<EdgeData>) => {
    setSelected(null);
